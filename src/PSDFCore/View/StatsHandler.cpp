...
#include <PSDFCore/View/StatsHandler.h>
...
StatsHandler::StatsHandler(OsgViewerBase* viewer):
    _statsType(NO_STATS),
    _initialized(false),
	_viewer(viewer),
    ...
    _statsHeight(1024.0f)
{
	...
	EventCenter::inst()->registerHandler(this, KEYBOARD_GROUP, _viewer->getStationId());
}

void StatsHandler::handleSystemEvent(unsigned eventType, int param1, int param2)
{
	if (!_viewer) return;
	
    if (_viewer && _threadingModelText.valid() && _viewer->getThreadingModel()!=_threadingModel)
    {
        _threadingModel = _viewer->getThreadingModel();
    ...
    switch(eventType)
	{
        case(KEY_DOWN):
        {
            ...
            {
                if (_viewer->getViewerStats())
                {
                    if (!_initialized)
                    {
                        setUpHUDCamera(_viewer);
                        setUpScene(_viewer);
                    }
                    ...
                    switch(_statsType)
                    {
                        case(NO_STATS):
                        {
                            _viewer->getViewerStats()->collectStats("frame_rate",false);
                            _viewer->getViewerStats()->collectStats("event",false);
                            _viewer->getViewerStats()->collectStats("update",false);
                            ...
                            _viewer->getViewerStats()->collectStats("scene",false);
                        ...
                        case(VIEWER_STATS):
                        {
                            ...
                            _viewer->getViewerStats()->collectStats("event",true);
                            _viewer->getViewerStats()->collectStats("update",true);
